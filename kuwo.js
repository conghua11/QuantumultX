...
if (url.indexOf(Play_URL1) !== -1) {
    ...
    !(async () => {
        ...
        await fetch('http://mobi.kuwo.cn/mobi.s?f=web&source=kwplayer_ar_1.1.9_oppo_118980_320.apk&type=convert_url_with_sign&br=' + br[i].bitrate + '&rid=' + rid
            ...
            const text1 = Object.keys(body).map(key => `${key}=${(body[key])}`).join(' ');
            console.log(text1)
            $.done({text: text1});
            })
    })().then(() => $.done());         
...
if (url.endsWith(KuWo_Down)) {
    ...
    obj = $.toObj($response.body);
    if ("undefined" !== typeof obj.songs[0].id) {
        id = obj.songs[0].id;
        KuWo.PlayID = id;
        KuWo.PlayType = "song";
        $.setval($.toStr(KuWo), 'KuWo');
...
if (url.match(KuWo_Vip)) {
    ...
    obj.data["vipmExpire"] = "4099817053315";
    obj.data["vipLuxuryExpire"] = "4099817053315";
    obj.data["svipExpire"] = "4099817053315";
